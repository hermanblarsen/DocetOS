 ...
 *           will enable the ASSERT_DEBUG(X) statement that sets a breakpoint if !x
 *       DEBUG_HARD
 ...
 *   Not defining a debug mode results in DPRINT or ASSERT_DEBUG statements being
...
/* ASSERT_DEBUG Breakpoint statement */
# define ASSERT_DEBUG(x) do{if(!(x))__breakpoint(0);}while(0)
